...
def analyze_rows(rows: List[Dict[str, Any]]) -> Tuple[Dict[str, Any], List[str]]:
    ...
    # Lag and Rolling window metrics
    productivity_lag_1 = productivity_series[-2] if len(productivity_series) >= 2 else (productivity_series[-1] if productivity_series else 0.0)
    focus_rolling_mean_3 = safe_average(focus_series[-3:]) if focus_series else 0.0
    productivity_rolling_mean_3 = safe_average(productivity_series[-3:]) if productivity_series else 0.0
    ...
    metrics = {
        ...
        "productivityLag1": productivity_lag_1,
        "focusRollingMean3": focus_rolling_mean_3,
        "productivityRollingMean3": productivity_rolling_mean_3,
        "analysisModels": [
    ...
    insights = [
        f"Productivity is {metrics['trendLabel'].lower()} with a projected next-window score of {round(predicted_next)} project accuracy.",
